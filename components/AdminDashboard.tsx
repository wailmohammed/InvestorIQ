...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUser, brokerages, setBrokerages, wallets, setWallets }) => {
  ...
  const handleAddWallet = () => {
    if(newWalletLabel && newWalletAddress) {
      setWallets([...wallets, {
        id: Date.now().toString(),
        network: newWalletNetwork || 'Unknown',
         ...
         <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 animate-fade-in">
            ...
            <div className="space-y-4">
                 ...
                 <div key={w.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 border border-slate-200 rounded-xl bg-slate-50">
                    <div className="flex items-start gap-4">
                       ...
                       <div>
                          ...
                          <p className="text-sm text-slate-500 font-mono">{w.address}</p>
                          <span className="inline-block mt-1 text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded">{w.network}</span>
                       </div>
                    </div>
              ...
              <div className="mt-6 pt-6 border-t border-slate-100">
                 <h4 className="font-bold text-slate-700 mb-4">Add New Wallet</h4>
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <input 
                      ...
                      className="border border-slate-200 rounded-lg px-4 py-2 text-sm font-mono"
                      value={newWalletAddress}
                      onChange={e => setNewWalletAddress(e.target.value)}
                    />
                 </div>
                 <button onClick={handleAddWallet} className="bg-slate-900 text-white px-6 py-2 rounded-lg text-sm font-bold flex items-center gap-2">
